...
export class AudioSystem {
  constructor(sceneEl) {
    ...
    const applyHackInterval = setInterval(() => {
      if (this.audioContext.state === "running") {
        this.applyAECHack();
        clearInterval(applyHackInterval);
      }
    }, 1000);
    ...
    waitForDOMContentLoaded().then(() => {
      if (sceneEl.is("entered")) {
        ...
        document.querySelector("#avatar-rig").addEventListener("entered", () => {
          // Start lip syncing after level loads to avoid hitching.
          if (supportsLipSync()) {
            this.startLipSync(sceneEl);
          }
    ...
    this.audioContext.audioWorklet.addModule(audioForwardWorkletSrc).then(() => {
      this.audioContext.audioWorklet.addModule(vadWorkletSrc).then(() => {
        ...
        const handleStateChange = e => {
          if (e.detail === "muted") {
            !this.scene.is("muted") ? this.enableLipSync() : this.disableLipSync();
          }
        };
