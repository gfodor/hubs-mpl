...
export class CharacterControllerSystem {
  ...
  tick = (function() {
    ...
    return function tick(t, dt) {
      ...
      const isMirroring = getCurrentlyMirroredMedia() !== null;
      ...
      if (characterAcceleration) {
        ...
        this.relativeMotionValue =
          Math.max(Math.abs(characterAcceleration[0]), Math.abs(characterAcceleration[1])) * (boost ? 2 : 1);
      } else {
        this.relativeMotionValue = 0;
      }
        ...
        if (triedToMove) {
          ...
          calculateDisplacementToDesiredPOV(
            ...
            this.relativeMotion.multiplyScalar(
              ((boost ? 2 : 1) * speedModifier * BASE_SPEED * Math.sqrt(playerScale) * dt) / 1000
            ),
            displacementToDesiredPOV
          );
