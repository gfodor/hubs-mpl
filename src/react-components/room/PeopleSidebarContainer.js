...
function PeopleListContainer({ hubChannel, people, onSelectPerson, onClose }) {
  const onMuteAll = useCallback(() => hubChannel.toggleMuteAll(), [hubChannel]);
...
export function PeopleSidebarContainer({
  ...
  if (selectedPerson) {
    if (selectedPerson.id === mySessionId) {
      // HACK
      setTimeout(() => AFRAME.scenes[0].systems["hubs-systems"].avatarSystem.markPersonaAvatarDirty(mySessionId), 500);
