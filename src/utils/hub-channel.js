...
export default class HubChannel extends EventTarget {
  constructor(store, hubId) {
    ...
    this.isMutingAll = false;
    this._muteAllInterval = null;
  }
  ...
  toggleMuteAll() {
    clearInterval(this._muteAllInterval);

    if (!this.isMutingAll) {
      this._muteAllInterval = setInterval(() => {
        const micPresences = getMicrophonePresences();

        for (const [id, micPresence] of micPresences.entries()) {
          if (!id) continue;
          if (id === NAF.clientId) continue;
          if (micPresence.muted) continue;

          const presence =
            this.presence && this.presence.state && this.presence.state[id] && this.presence.state[id].metas[0];

          if (presence && presence.presence === "room" && presence.permissions && !presence.permissions.mute_users) {
            this.mute(id);
          }
        }
      }, 250);
    ...
    this.isMutingAll = !this.isMutingAll;
    this.dispatchEvent(new CustomEvent("mute_all_changed"));
  }
  ...
  sendEnteredEvent = async () => {
    ...
    const state =
      ...
      window.APP.hubChannel.presence.state[NAF.clientId];
    const profile = state && state.metas[0] && state.metas[0].profile;
    ...
    const entryEvent = {
      ...
      communityIdentifier: profile && profile.communityIdentifier,
      displayName: profile && profile.displayName,
      identityName: profile && profile.identityName,
      hubId: this.hubId
    };
