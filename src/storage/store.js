...
const LOCAL_STORE_KEY = "___ostn_store";
  ...
  definitions: {
    profile: {
      ...
      properties: {
        ...
        avatarPrimaryR: { type: "number" },
        avatarPrimaryG: { type: "number" },
        avatarPrimaryB: { type: "number" }
      }
    },
...
export default class Store extends EventTarget {
  ...
  initProfile = async () => {
    ...
    if (!this.state.activity.hasChangedName) {
      const avatarPrimaryR =
        this.state.profile.avatarPrimaryR !== undefined ? this.state.profile.avatarPrimaryR : Math.random();
      const avatarPrimaryG =
        this.state.profile.avatarPrimaryG !== undefined ? this.state.profile.avatarPrimaryG : Math.random();
      const avatarPrimaryB =
        this.state.profile.avatarPrimaryB !== undefined ? this.state.profile.avatarPrimaryB : Math.random();

      this.update({
        profile: { displayName: generateRandomName(), avatarPrimaryR, avatarPrimaryG, avatarPrimaryB }
      });
    }
  };
