...
export default class SceneEntryManager {
  ...
  _runBot = async () => {
    // Spawn bot on a random X, Z
    setTimeout(() => {
      const pos = new THREE.Vector3();
      this.avatarRig.object3D.getWorldPosition(pos);
      pos.x += (Math.random() - 0.5) * 40.0;
      pos.z += (Math.random() - 0.5) * 40.0;

      this.avatarRig.object3D.position.x = pos.x;
      this.avatarRig.object3D.position.z = pos.z;
    ...
    if (audioStream) {
      ...
      for (const track of this.mediaDevicesManager.mediaStream.getAudioTracks()) {
        this.mediaDevicesManager.mediaStream.removeTrack(track);
      }
