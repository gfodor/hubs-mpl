...
AFRAME.registerComponent("networked-drawing", {
  ...
  tick(t) {
    ...
    if (!this.connectedToOwner && connected) {
      ...
      if (!isMine && NAF.connection.hasActiveDataChannel(owner)) {
        NAF.connection.sendCustomDataGuaranteed(
          this.drawingId,
          {
  ...
  _broadcastDrawing: (() => {
    ...
    return function() {
      ...
      NAF.connection.broadcastCustomDataGuaranteed(this.drawingId, { type: MSG_BUFFER_DATA, buffer: copyArray });
    };
  })(),
